        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div>
            <h2 className="text-5xl font-bold leading-tight">
              La presse qui<br/>fait avancer<br/><span className="text-primary-foreground/90">la Maison First.</span>
            </h2>
            <p className="mt-6 text-white/80 max-w-md">
              Accédez à toute la presse économique et financière, suivez en direct les règlementations COBAC et CEMAC, recevez les alertes stratégiques.
            </p>
      ...
      <div className="flex flex-col p-6 sm:p-10 bg-card">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto">
          ...
          <div className="mt-6 space-y-2">
            <button onClick={() => oauth("google")} className="w-full h-11 rounded-md border border-border bg-background hover:bg-accent flex items-center justify-center gap-2 text-sm font-medium">
              <GoogleIcon /> {t("continueGoogle")}
            </button>
            <button onClick={() => oauth("apple")} className="w-full h-11 rounded-md border border-border bg-background hover:bg-accent flex items-center justify-center gap-2 text-sm font-medium">
          ...
          <form onSubmit={submit} className="space-y-3">
            {mode === "signup" && (
              <label className="block">
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} className="input mt-1" placeholder="ex. Jean Dupont" />
              </label>
            )}
            <label className="block">
              <span className="text-xs text-muted-foreground">{t("email")}</span>
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" required className="input mt-1" placeholder="prenom.nom@afrilandfirstbank.com" />
            </label>
            <label className="block">
              <span className="text-xs text-muted-foreground">{t("password")}</span>
              <div className="relative mt-1">
                <input value={password} onChange={(e) => setPassword(e.target.value)} type={show ? "text" : "password"} required minLength={6}
...
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48">
      <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.6-6 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.1 7.9 3l5.7-5.7C34 6.1 29.3 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.4-.4-3.5z"/>
